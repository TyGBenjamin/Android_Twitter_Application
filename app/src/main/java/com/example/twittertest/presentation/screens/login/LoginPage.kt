...
import com.example.twittertest.toggleableLoginPrompt
...
fun LoginPage(
    ...
    LaunchedEffect(key1 = token) {
        when (token) {
            is Resource.Error -> {}
            Resource.Loading -> {}
            is Resource.Success -> navigate(ScreenConstants.dashboardScreen)
            else -> {}
        }
    }
    Column(
        ...
        Icon(
            painter = painterResource(id = R.drawable.twitter_logo), null,
