...
class PostViewModel @Inject constructor(
    ...
    private val _createPost: MutableStateFlow<Resource<Post>> = MutableStateFlow(Resource.Loading)
    val createPost = _createPost.asStateFlow()

    /**
     *
     */
    fun createPost(post: PostAdd) {
        viewModelScope.launch {

            val token = datastore.getPreferenceInfo().first()

            _createPost.value = repo.addPost("Bearer $token", post)
